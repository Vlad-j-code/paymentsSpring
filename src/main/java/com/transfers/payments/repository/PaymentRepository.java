...
public interface PaymentRepository extends JpaRepository<Payment, Integer> {
    ...
    @Query(value = "select * from payments where card_destination_id in (SELECT id from cards where user_id = ?) order by date desc", nativeQuery = true)
    Page<Payment> pageIncByUserId(int userId, Pageable pageable);

    @Query(value = "select * from payments where card_destination_id in (SELECT id from cards where user_id = ? and card_id != 0) order by date desc", nativeQuery = true)
    Page<Payment> pagePayByUserId(int userId, Pageable pageable);
}
